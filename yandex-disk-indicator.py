...
class Indicator(YDDaemon):      # Yandex.Disk appIndicator
  ...
  class Menu(Gtk.Menu):             # Indicator menu
    ...
    def update(self, vals, yddir):  # Update information in menu
      self.folder = yddir
      # Update status data
      self.status.set_label(_('Status: ') + self.YD_STATUS[vals['Stat']] +
                              (vals['Prog'] if vals['Stat'] == 'busy'
                               else
                               ' '.join((':', vals['Err'], shortPath(vals['ErrP']))) if vals['Stat'] == 'error'
                               else
                               ''))
      # Update sizes data
      ...
      if vals['ChLast']:
        ...
        for filePath in vals['Last']:            # Create new sub-menu items
...
if __name__ == '__main__':
  ...
  # Initialize notification engine for application messages (it is used in Preferences dialogue)
